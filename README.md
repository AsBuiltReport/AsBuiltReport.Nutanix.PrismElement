...
Nutanix Prism Element As Built Report is a PowerShell module which works in conjunction with [AsBuiltReport.Core](https://github.com/AsBuiltReport/AsBuiltReport.Core).

[AsBuiltReport](https://github.com/AsBuiltReport/AsBuiltReport) is an open-sourced community project which utilises PowerShell to produce as-built documentation in multiple document formats for multiple vendors and technologies.

The Nutanix Prism Element As Built Report module is used to generate as built documentation for Nutanix hyperconverged infrastructure.
...
![Sample Nutanix Prism Element As Built Report](https://github.com/AsBuiltReport/AsBuiltReport.Nutanix.PrismElement/blob/master/Samples/Sample%20Nutanix%20Prism%20Element%20As%20Built%20Report.png "Sample Nutanix Prism Element As Built Report")
...
Below are the instructions on how to install, configure and generate a Nutanix Prism Element As Built report.
...
>[!WARNING]
>This report module leverages the Nutanix v1 and v2 APIs, which are [scheduled for end of support in Q4 CY2026](https://download.nutanix.com/misc/LegacyAPI-EOLNotification.pdf). After that date, ongoing development of this module will likely cease.

## :no_entry_sign: Unsupported Versions
The following Nutanix Prism Element versions are no longer being tested and/or supported;
- AOS 5.x
- AOS 6.8 or earlier
...
The Nutanix Prism Element As Built Report utilises a JSON file to allow configuration of report information, options, detail and healthchecks.

A Nutanix Prism Element report configuration file can be generated by executing the following command;
```powershell
New-AsBuiltReportConfig -Report Nutanix.PrismElement -FolderPath <User specified folder> -Filename <Optional>
```

Executing this command will copy the default Nutanix Prism Element report configuration file to a user specified folder.
...
### Report
The **Report** schema provides configuration of the Nutanix Prism Element report information
...
| System         |        2        |        4        |
| Host           |        2        |        3        |
| Network<sup>**1**</sup>       |        0        |        3        |
| Storage        |        2        |        3        |
| VM             |        2        |        3        |
| DataProtection |        2        |        3        |

>[!Note]
><sup>**1**</sup> Experimental feature which is still in development
...
| TimezoneSetting | User Defined | UTC     | Checks the configured timezone for the Nutanix cluster   | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) Not compliant with user defined setting                                                                                              |
| DataResiliency  | true/false   | true    | Checks the data resiliency status of the Nutanix cluster | ![OK](https://place-hold.it/15/DFF0D0/DFF0D0) Data relisency status is possible <br>![Critical](https://place-hold.it/15/FEDDD7/FEDDD7) Data relisency status is not possible |
...
| Sub-Schema | Setting    | Default | Description                          | Highlight                                                                       |
| ---------- | ---------- | ------- | ------------------------------------ | ------------------------------------------------------------------------------- |
| PowerState | true/false | true    | Highlights if the CVM is powered off | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) CVM is powered off |
...
| Sub-Schema | Setting    | Default | Description                                        | Highlight                                                                                         |
| ---------- | ---------- | ------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| ImageState | true/false | false   | Highlights images which are in an inactive state   | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) Image is in an inactive state        |
| Licensing  | true/false | true    | Highlights if no license is applied to the cluster | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) No license is applied to the cluster |

#### Host
The **Host** schema is used to configure health checks for Nutanix host hardware.

| Sub-Schema | Setting    | Default | Description                                       | Highlight                                                                               |
| ---------- | ---------- | ------- | ------------------------------------------------- | --------------------------------------------------------------------------------------- |
| DiskStatus | true/false | true    | Highlights disks where their status is not normal | ![Critical](https://place-hold.it/15/FEDDD7/FEDDD7) Disk status is not normal |
| DiskMode   | true/false | true    | Highlights disks which are offline                | ![Critical](https://place-hold.it/15/FEDDD7/FEDDD7) Disk mode is offline      |
...
| Sub-Schema     | Setting    | Default | Description                                                                    | Highlight                                                                                       |
| -------------- | ---------- | ------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- |
| Compression    | true/false | false   | Highlights storage containers which do not have compression enabled            | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) Compression is disabled            |
| CacheDedupe    | true/false | false   | Highlights storage containers which do not have cache deduplication enabled    | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) Cache deduplication is disabled    |
| CapacityDedupe | true/false | false   | Highlights storage containers which do not have capacity deduplication enabled | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) Capacity deduplication is disabled |
| ErasureCoding  | true/false | false   | Highlights storage containers which do not have erasure coding enabled         | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) Erasure coding is disabled         |
...
| Sub-Schema         | Setting    | Default | Description                               | Highlight                                                                           |
| ------------------ | ---------- | ------- | ----------------------------------------- | ----------------------------------------------------------------------------------- |
| PowerState         | true/false | true    | Highlights VMs which are powered off      | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) VM is powered off      |
| NicConnectionState | true/false | true    | Highlights VM NICs which are disconnected | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) VM NIC is disconnected |
...
| Sub-Schema          | Setting    | Default | Description                                    | Highlight                                                                                     |
| ------------------- | ---------- | ------- | ---------------------------------------------- | --------------------------------------------------------------------------------------------- |
| CompressOnWire      | true/false | false   | Highlights if line compression is disabled     | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) Line compression is disabled     |
| BandwidthThrottling | true/false | false   | Highlights if bandwidth throttling is disabled | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) Bandwidth throttling is disabled |
| Proxy               | true/false | false   | Highlights if proxy setting is disabled        | ![Warning](https://place-hold.it/15/FFF4C7/FFF4C7) Proxy setting is disabled        |
